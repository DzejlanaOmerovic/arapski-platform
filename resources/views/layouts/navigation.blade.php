<nav class="main-nav">
    ...
    <div class="nav-links">
        ...
        @auth
            @if(Auth::user()->role === 'teacher')
                ...
                <a href="{{ route('teacher.profile') }}" {{ request()->routeIs('teacher.profile') ? 'class=active' : '' }}>Moj
                    profil</a>
            ...
            @elseif(Auth::user()->role === 'admin')
                ...
                <a href="{{ route('admin.notifications') }}" {{ request()->routeIs('admin.notifications') ? 'class=active' : '' }}>Dodaj obavještenja</a>
                <a href="{{ route('admin.settings') }}" {{ request()->routeIs('admin.settings') ? 'class=active' : '' }}>Podešavanja</a>
            @endif
            {{-- Profil i lozinka za SVE korisnike --}}
            <a href="{{ route('profile.edit') }}" {{ request()->routeIs('profile.edit') ? 'class=active' : '' }}>⚙️
                Profil</a>
        @endauth
    </div>
    ...
    <div class="nav-user">
        ...
        @else
            <a href="{{ route('login') }}"
                style="color:rgba(255,255,255,0.7);text-decoration:none;padding:7px 16px;border-radius:8px;font-size:0.83rem;font-weight:600;border:1px solid rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);transition:all 0.2s;">Prijava</a>
            <a href="{{ route('register') }}"
                style="background:linear-gradient(135deg,#6366f1,#a855f7);color:white;text-decoration:none;padding:7px 16px;border-radius:8px;font-size:0.83rem;font-weight:700;box-shadow:0 4px 14px rgba(99,102,241,0.4);transition:all 0.2s;">Registracija
                →</a>
        @endauth
    </div>
</nav>